  ...
  server.registerTool(
    "resolve_version",
    {
      ...
      description: [
        ...
        "- To resolve the current MDS web wrapper version when the user wants the higher-level document scanner on top of DCV JS.",
        ...
        "",
        "RETURNS: A text block showing the resolved version. For DCV/DBR without an edition, returns all edition versions. For DWT/DDV/MDS, returns the single web version.",
        "",
        "EXAMPLE: resolve_version with product='dbr', edition='web' returns the latest DBR web SDK version string. resolve_version with product='mds' returns the latest MDS web wrapper version.",
        "",
    ...
    async ({ product, edition, platform, constraint, feature }) => {
      ...
      if (!["dcv", "dbr", "dwt", "ddv", "mds"].includes(normalizedProduct)) {
        return {
          isError: true,
          content: [{ type: "text", text: `Unknown product "${product}". Use dcv, dbr, dwt, ddv, or mds.` }]
        };
      }
      ...
      if (normalizedProduct === "mds") {
        if (normalizedEdition && normalizedEdition !== "web") {
          return {
            isError: true,
            content: [{ type: "text", text: `MDS only supports the web edition in this MCP server. Unsupported edition: "${normalizedEdition}".` }]
          };
        }
        const lines = [
          "# MDS Version Resolution",
          `- Latest major: v${LATEST_MAJOR.mds}`,
          "- Product model: higher-level document scanner wrapper on top of DCV JS",
          `- Resolved version: ${LATEST_VERSIONS.mds.web}`
        ];
        return { content: [{ type: "text", text: lines.join("\n") }] };
      }
